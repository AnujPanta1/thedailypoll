	...
	useEffect(
		() => {
			...
			const updateUserVotes = async () => {
				...
				let newVotedPolls = Object.assign(userData["votedPolls"], {
					[currentPollId]: choosenVote,
				});
				let sendOffData = { ...userData, votedPolls: newVotedPolls };
				await setDoc(
					doc(db, "users", getCookie("userKey")),
					sendOffData
				);
			};
	...
	return (
		<div className="graph-interactive-wrapper">
			<InteractiveSection
				...
				options={Object.keys(currentPollData["votes"])}
				choosenVote={choosenVote}
				setChoosenVote={setChoosenVote}
				setVoted={setVoted}
				hasVoted={voted}
			/>
			<DataGraph
				...
				answered={choosenVote !== "?" ? true : false}
				choosenIndex={Object.keys(currentPollData["votes"]).indexOf(
					choosenVote
				)}
			></DataGraph>
		</div>
